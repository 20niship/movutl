...
class LuaIntfWriter:
    ...
    def set(
        ...
        # Entity基底のgroup="transform"プロパティ(pos_/anchor_等)を、描画変換を持つ派生クラスにもLuaから見せる(LuaIntfは継承を辿らない)
        ent = next((c for c in classes_list if c.name == "Entity"), None)
        self.transform_props = [p for p in ent.props if p.group == "transform"] if ent else []
    ...
    def register_class(self, cls: MClass):
        ...
        if cls.name in TRANSFORM_ENTITY_CLASSES:
