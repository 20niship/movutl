...
namespace {
Composition make_test_comp(int w, int h) {
  Composition comp("test", w, h, 30);
  comp.frame_final = cutil::make_ref<Image>();
  comp.frame_final->resize(w, h);
  comp.frame_final->has_alpha = false;
  comp.frame_final->fill_rgba(Vec4b(0, 0, 0, 255));
  return comp;
}
} // namespace
...
TEST_CASE("ShapeEntt::render 四角形を指定位置・指定色で描画する") {
  ...
  CHECK(comp.frame_final->rgba(5, 5) == Vec4b(0, 0, 0, 255));     // 矩形の外側は背景のまま
}
...
TEST_CASE("ShapeEntt::render 枠線を指定色・太さで描画する") {
  ...
  CHECK(comp.frame_final->rgba(10, 20) == Vec4b(0, 255, 0, 255)); // 左端の枠線
  CHECK(comp.frame_final->rgba(1, 1) == Vec4b(0, 0, 0, 255));     // 枠線の外は背景のまま
}
...
TEST_CASE("ShapeEntt::render 円は矩形の隅を塗らない") {
  ...
  CHECK(comp.frame_final->rgba(25, 25) == Vec4b(0, 255, 0, 255)); // 円の中心
  CHECK(comp.frame_final->rgba(1, 1) == Vec4b(0, 0, 0, 255));     // バウンディングボックスの隅は円の外
}
...
TEST_CASE("ShapeEntt::render カスタムパスは点群のbboxに合わせて配置される") {
  ...
  CHECK(comp.frame_final->rgba(1, 1) == Vec4b(0, 0, 0, 255));     // パス外
}
