...
namespace {
// ImageはEntity派生でコピー不可のためRefで返す
Ref<Image> make_src(int w, int h) {
  auto s       = cutil::make_ref<Image>(w, h);
  s->has_alpha = true;
  for(size_t i = 0; i < s->size(); i++) (*s)[i] = Vec4b(255, 0, 0, 255);
  return s;
}
Ref<Image> make_target(int w, int h) {
  auto t       = cutil::make_ref<Image>(w, h);
  t->has_alpha = true;
...
TEST_CASE("Entity::composite: 縦横比aspectが正だと横が縮み、負だと縦が縮む") {
  ...
  REQUIRE(ent->composite(*src, u.get()));
