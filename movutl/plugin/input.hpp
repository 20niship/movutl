...
struct InputPluginTable {
  ...
  bool (*fn_info_get)(InputHandle ih, InputInfo* iip);                       //	入力ファイルの情報を取得する関数へポインタ
  int (*fn_set_frame)(InputHandle ih, int frame);                            //	フレームを設定する関数へのポインタ
  int (*fn_get_frame)(InputHandle ih);                                       //	現在のフレーム番号を取得する関数へのポインタ
  int (*fn_read_video)(InputHandle ih, const InputInfo* iip, Movie* entity); //	画像データを読み込む関数へのポインタ
  //	ih		: 入力ファイルハンドル
  //	frame	: 読み込むフレーム番号
