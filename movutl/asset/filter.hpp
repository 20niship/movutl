...
typedef struct {
  ...
  //	flag 	: 読み込みフラグ
  //	AVI_FILE_OPEN_FLAG_ONLY_YUY2		: YUY2 で展開するようにしす
...
typedef struct {
  ...
  TCHAR** track_name;     //	トラックバーの名前郡へのポインタ(トラックバーが0ならNULLでよい)
  ...
  //	WM_FILTER_UPDATE		: 各フィルタ設定や編集内容が変更れた直後に送られます
  ...
  //	WM_FILTER_CHANGE_WINDOW	: フィルタウィンドウの表示/表示が変更された直後に送られます
  ...
  //	WM_FILTER_MAIN_KEY_???のwparamには仮想キーコードが入りす
