...
//		入力プラグイン構造体定義
//---------------------------------------------------------------------
mu::InputPluginTable input_plugin_table = {
  ...
  fn_is_keyframe,                                    //	キーフレームか調べる関数へのポインタ (NULLなら全てキーフレーム)
  fn_config,                                         //	入力設定のダイアログを要求された時に呼ばれる関数へのポインタ (NULLなら呼ばれません)
};
...
//		ファイルオープン
//---------------------------------------------------------------------
INPUT_HANDLE fn_open(LPSTR file) {
  FILE_HANDLE* fp;
  int i;
  PAVISTREAM pas;
  AVISTREAMINFO asi;
...
//		ファイルの情報
//---------------------------------------------------------------------
bool fn_info_get(INPUT_HANDLE ih, INPUT_INFO* iip) {
  FILE_HANDLE* fp = (FILE_HANDLE*)ih;
...
//		画像読み込み
//---------------------------------------------------------------------
int fn_read_video(INPUT_HANDLE ih, int frame, void* buf) {
  FILE_HANDLE* fp = (FILE_HANDLE*)ih;
  LONG videosize;
  LONG size;
...
//		音声読み込み
//---------------------------------------------------------------------
int fn_read_audio(INPUT_HANDLE ih, int start, int length, void* buf) {
  FILE_HANDLE* fp = (FILE_HANDLE*)ih;
  LONG size;
  int samplesize;
...
//		キーフレーム情報
//---------------------------------------------------------------------
bool fn_is_keyframe(INPUT_HANDLE ih, int frame) {
  FILE_HANDLE* fp = (FILE_HANDLE*)ih;
...
//		設定ダイアログ
//---------------------------------------------------------------------
bool fn_config(HWND hwnd, HINSTANCE dll_hinst) {
